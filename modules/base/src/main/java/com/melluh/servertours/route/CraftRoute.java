...
public class CraftRoute implements Route {
    ...
    public CraftRoute(ConfigurationSection configurationSection) {
        ...
        this.positionInterpolationMode = readEnum(
                configurationSection,
                "camera.positionInterpolation",
                PositionInterpolationMode.class,
                PositionInterpolationMode.CENTRIPETAL_CATMULL_ROM
        );
        this.rotationInterpolationMode = readEnum(
                configurationSection,
                "camera.rotationInterpolation",
    ...
    @Override
    public PositionInterpolationMode getPositionInterpolationMode() {
        return this.positionInterpolationMode;
    }

    @Override
    public void setPositionInterpolationMode(PositionInterpolationMode mode) {
        PositionInterpolationMode required = Objects.requireNonNull(mode, "position interpolation mode may not be null");
        if (this.positionInterpolationMode == required) {
            return;
        }
        this.positionInterpolationMode = required;
        this.recalculateSplines();
    }
    ...
    @Override
    public void setRotationInterpolationMode(RotationInterpolationMode mode) {
        RotationInterpolationMode required = Objects.requireNonNull(mode, "rotation interpolation mode may not be null");
        if (this.rotationInterpolationMode == required) {
            return;
        }
    ...
    public <T extends CraftRoutePoint> List<T> getPointsOfType(Class<T> clazz) {
        List<T> matchingPoints = this.points.stream()
                .filter(clazz::isInstance)
                .map(clazz::cast)
                .collect(Collectors.toCollection(ArrayList::new));
        Collections.reverse(matchingPoints);
    ...
    private static <E extends Enum<E>> E readEnum(ConfigurationSection section, String path,
                                                   Class<E> enumType, E defaultValue) {
        String configured = section.getString(path);
        ...
        try {
            ...
            throw new IllegalArgumentException("Invalid " + path + " value '" + configured
                    + "' for route '" + section.getString("name", "<unnamed>") + "'", exception);
        }
    }
}
